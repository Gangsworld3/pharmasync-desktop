...
def _conflict_payload(conflict_type: str, entity_name: str, entity_id: str, change: dict[str, Any], server: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = {
        "type": conflict_type,
        "entity": entity_name,
        "entityId": entity_id,
...
def _appointment_suggestions(session: Session, staff_name: str, starts_at: datetime, ends_at: datetime, limit: int = 3) -> list[str]:
...
def _apply_generic_change(session: Session, change: dict[str, Any], payload: dict[str, Any], existing: Any | None, device_id: str) -> tuple[Any, int]:
...
def handle_sync_push(session: Session, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for change in payload["changes"]:
        ...
        if duplicate:
            results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "IDEMPOTENT_REPLAY", "serverRevision": duplicate.server_revision})
            continue
        ...
        if existing_conflict:
            ...
            results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "CONFLICT", "resolution": "REQUIRES_USER_ACTION"})
            continue
        ...
        try:
            with session.begin_nested():
                ...
                if existing and local_revision < existing.server_revision:
                    if entity_name == "Client" and operation in {"UPDATE", "CREATE"}:
                        ...
                        event = append_sync_event(session, entity=entity_name, operation="UPDATE", entity_id=entity_id, payload={"merged_fields": merged_fields, "data": data}, operation_id=operation_id, device_id=device_id, resolution_type="AUTO_MERGED", resolved=True)
                        ...
                        conflicts.append({"type": "CLIENT_MERGE", "entity": entity_name, "entityId": entity_id, "local": change, "server": existing.model_dump(mode="json"), "resolution": "AUTO_MERGED", "mergedFields": merged_fields})
                        results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "APPLIED", "resolution": "AUTO_MERGED"})
                        continue
                    ...
                    if entity_name == "Message":
                        ...
                        enqueue_conflict(session, operation_id=operation_id, entity_type=entity_name, entity_id=entity_id, conflict_type="CONFLICT", payload=conflict)
                        conflicts.append(conflict)
                        results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "CONFLICT"})
                        continue

                if entity_name == "Appointment" and operation in {"CREATE", "UPDATE"} and data.get("staff_name"):
                    overlap = find_appointment_conflict(session, data["staff_name"], data["starts_at"], data["ends_at"], exclude_id=entity_id if existing else None)
                    if overlap:
                        conflict = _conflict_payload("APPOINTMENT_OVERLAP", entity_name, entity_id, change, overlap.model_dump(mode="json"))
                        conflict["suggestions"] = _appointment_suggestions(session, data["staff_name"], data["starts_at"], data["ends_at"])
                        enqueue_conflict(session, operation_id=operation_id, entity_type=entity_name, entity_id=entity_id, conflict_type="APPOINTMENT_OVERLAP", payload=conflict)
                        conflicts.append(conflict)
                        results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "CONFLICT", "resolution": "REQUIRES_USER_ACTION"})
                        continue

                if entity_name == "Invoice" and operation == "CREATE":
                    invoice = Invoice(id=entity_id, invoice_number=data["invoice_number"], client_id=data.get("client_id"), currency_code=data.get("currency_code", "SSP"), total_minor=0, balance_due_minor=0, payment_method=data["payment_method"], status=data.get("status", "ISSUED"))
                    create_invoice(session, invoice, _parse_invoice_items(data), device_id=device_id, operation_id=operation_id, auto_commit=False)
                ...
                if entity_name == "Message" and operation == "CREATE":
                    message = MessageEvent(id=entity_id, conversation_id=data["conversation_id"], sender_id=data["sender_id"], content=data["content"], created_at=data.get("created_at") or datetime.now(UTC))
                    session.add(message)
                    session.flush()
                    event = append_sync_event(session, entity="Message", operation="CREATE", entity_id=entity_id, payload=message.model_dump(mode="json"), operation_id=operation_id, device_id=device_id)
                ...
                if entity_name == "InventoryItem" and operation in {"CREATE", "UPDATE"} and "quantity_on_hand" in data and Decimal(str(data["quantity_on_hand"])) < 0:
                    conflict = _conflict_payload("INSUFFICIENT_STOCK", entity_name, entity_id, change, existing.model_dump(mode="json") if existing else None)
                    enqueue_conflict(session, operation_id=operation_id, entity_type=entity_name, entity_id=entity_id, conflict_type="INSUFFICIENT_STOCK", payload=conflict)
                    conflicts.append(conflict)
                    results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "REJECTED", "resolution": "REQUIRES_USER_ACTION"})
                    continue
            ...
            if entity_name != "Message":
                enqueue_conflict(session, operation_id=operation_id, entity_type=entity_name, entity_id=entity_id, conflict_type=conflict_type, payload=conflict_payload)
            conflicts.append(conflict_payload)
            results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "REJECTED", "error": str(exc), "resolution": "REQUIRES_USER_ACTION"})
        except IntegrityError:
            session.rollback()
            existing = get_sync_event_by_operation_id(session, operation_id)
            if existing:
                results.append({"operationId": operation_id, "entity": entity_name, "entityId": entity_id, "status": "IDEMPOTENT_REPLAY", "serverRevision": existing.server_revision})
            else:
    ...
    server_changes = [{"serverRevision": event.server_revision, "entity": event.entity, "operation": event.operation, "entityId": event.entity_id, "data": json.loads(event.payload_json), "resolutionType": event.resolution_type, "resolved": event.resolved} for event in list_sync_events_since(session, payload.get("lastPulledRevision", 0))]
    ...
    return {"newRevision": new_revision, "applied": applied, "conflicts": conflicts, "serverChanges": server_changes, "results": results}


def handle_sync_pull(session: Session, since: int) -> dict[str, Any]:
    changes = [{"serverRevision": event.server_revision, "entity": event.entity, "operation": event.operation, "entityId": event.entity_id, "data": json.loads(event.payload_json)} for event in list_sync_events_since(session, since)]
